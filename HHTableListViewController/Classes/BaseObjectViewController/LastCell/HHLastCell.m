...
- (void)setLayout {
    ...
    _emptyView = [[HHEmptyView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, ErrorViewHeight) image:[UIImage imageNamed:@"icon_empty_data"] andMessage:nil];
...
- (void)setStatus:(LastCellStatus)status
{
    ...
    if (status == LastCellStatusEmpty) {
        self.frame = _emptyView.bounds;
        _emptyView.hidden = NO;
        _emptyView.message = _emptyMessage;
    } else if (!_emptyView.hidden) {
